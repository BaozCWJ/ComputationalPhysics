...
def soli1(dx,dt,delta,ini):
    ...
    u=np.zeros(Num)
    for j in range(0,Num):
        u[j%Num]=ini[j%Num]-dt/dx*((ini[(j+1)%Num]-ini[(j-1)%Num])/6*(ini[(j+1)%Num]+ini[j%Num]+ini[(j-1)%Num])+delta*delta/dx*(ini[(j+2)%Num]-2*ini[(j+1)%Num]+2*ini[(j-1)%Num]-ini[(j-2)%Num])/dx/2)
    return u
#选取合适的参数绘制图形
